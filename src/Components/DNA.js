  ...
  return (
    <div className="dna-testing-page">
      ...
      <section className="procedures-section">
        <div className="section-header">
          <span className="section-label">{t.procedures.sectionLabel}</span>
          <h2>{t.procedures.title}</h2>
          <p className="section-description">{t.procedures.description}</p>
        </div>
        <div className="procedures-timeline">
          {t.procedures.steps.map((procedure, index) => (
            <div key={index} className="procedure-step">
              <div className="step-connector">
                {index < t.procedures.steps.length - 1 && <div className="connector-line"></div>}
              </div>
              <div className="step-content">
                ...
                <div className="step-details">
                  ...
                  <ul className="step-list">{procedure.details.map((d, i) => <li key={i}>{d}</li>)}</ul>
                </div>
              </div>
            </div>
          ))}
      ...
      <section className="impacts-section">
        <div className="section-header">
          <span className="section-label">{t.impacts.sectionLabel}</span>
          <h2>{t.impacts.title}</h2>
          <p className="section-description">{t.impacts.description}</p>
        </div>
        <div className="impacts-grid">
          {t.impacts.items.map((impact, index) => (
            <div key={index} className="impact-card">
              <div className="impact-icon">{impact.icon}</div>
              <h3>{impact.title}</h3>
              <p>{impact.description}</p>
              <div className="impact-benefits">
                {impact.benefits.map((b, i) => <div key={i} className="benefit-tag">{b}</div>)}
              </div>
            </div>
          ))}
        </div>
